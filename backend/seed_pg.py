"""
Seed-скрипт: 10 уроков A1 для PostgreSQL (asyncpg)
...
Запуск:
    ...
    python seed_pg.py
  или с явным URL:
    DATABASE_URL=postgresql://... python seed_pg.py
"""
...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql://talk2learn_user:c2oYAlQh6g9RtcWHyOrUVn9lHgN4pSG4@dpg-d8qi41ugvqtc73a9bn4g-a.oregon-postgres.render.com/talk2learn"
)
...
💡 <b>Совет:</b> Запомните: «on» используется с днями, «in» — с месяцами!""",
        [
            ...
            ("Which preposition do we use with days? '___ Monday'", "in", "at", "on", 3),
        ]
    ),
    (
        8,
        ...
        for order_num, title, lesson_text, questions in LESSONS:
            ...
            if existing:
                ...
                lesson_id = await conn.fetchval("""
                    ...
                    VALUES ('A1', $1, $2, 'lesson', $3, TRUE, 'en')
                    RETURNING id
